		...
		@years.keys.sort.reverse_each do |year|
			@years[year].sort.reverse_each do |month|
				...
				m.diaries.keys.sort.reverse_each do |date|
					...
					result << %Q|<li><a href="#{@index}#{anchor date}">#{@conf.shorten( DiaryBase.method_defined?(:stripped_title) ? m.diaries[date].stripped_title : m.diaries[date].title, limit )}</a></li>\n|
					max -= 1
					throw :exit if max == 0
				end
			end
